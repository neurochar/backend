...
func (r *Repository) buildWhereForList(listOptions *usecase.AccountListOptions, withDeleted bool) (where squirrel.And) {
	defer func() {
		if !withDeleted {
...
func (r *Repository) FindList(
	...
	result := make([]*entity.Account, 0, len(dbData))
...
func (r *Repository) FindPagedList(
	...
	totalQ := r.qb.Select("COUNT(*) as total").From(pg.AccountTable).Where(where)
...
func (r *Repository) FindOneByID(
	...
	q := r.qb.Select(pg.AccountTableFields...).From(pg.AccountTable).Where(where)
...
func (r *Repository) FindOneByEmail(
	...
	where := squirrel.And{
		squirrel.Eq{"tenant_id": tenantID, "email": email},
	}
