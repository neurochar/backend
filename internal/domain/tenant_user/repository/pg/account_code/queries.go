...
func (r *Repository) FindList(
	...
	fields := pg.AccountCodeTableFields

	q := r.qb.Select(fields...).From(pg.AccountCodeTable).Where(where)
	...
	result := make([]*entity.AccountCode, 0, len(dbData))
