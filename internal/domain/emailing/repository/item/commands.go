...
func (r *Repository) Create(ctx context.Context, item *entity.Item) error {
	...
	query, args, err := r.qb.Insert(Table).SetMap(dataMap).Suffix("RETURNING id").ToSql()
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, item *entity.Item) error {
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		checkQuery, checkArgs, err := r.qb.Select("id").From(Table).Where(squirrel.Eq{"id": item.ID}).ToSql()
		if err != nil {
		...
		updQuery, updArgs, err := r.qb.Update(Table).Where(
...
func (r *Repository) DeleteByID(ctx context.Context, id uuid.UUID) error {
	...
	query, args, err := r.qb.Delete(Table).Where(squirrel.Eq{"id": id}).ToSql()
	if err != nil {
		r.logger.ErrorContext(loghandler.WithSource(ctx), "building query", slog.Any("error", err))
		return appErrors.Chainf(appErrors.ErrInternal.WithWrap(err), "%s.%s", r.pkg, op)
	}
