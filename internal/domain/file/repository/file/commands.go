...
func (r *Repository) Create(ctx context.Context, item *fileEntity.File) error {
	const op = "Create"
...
func (r *Repository) Update(ctx context.Context, item *fileEntity.File) error {
	const op = "Update"
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		checkQuery, checkArgs, err := r.qb.Select("id").From(FileTable).Where(squirrel.Eq{"id": item.ID}).ToSql()
		if err != nil {
		...
		updQuery, updArgs, err := r.qb.Update(FileTable).Where(
