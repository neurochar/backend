...
func (uc *UsecaseImpl) FindOneByID(
	...
	dto, err := uc.entitiesToDTO(ctx, []*entity.Room{item}, dtoOpts)
	if err != nil {
		return nil, appErrors.Chainf(err, "%s.%s", uc.pkg, op)
	}
...
func (uc *UsecaseImpl) FindListInMap(
	...
	queryParams *uctypes.QueryGetListParams,
	dtoOpts *usecase.RoomDTOOptions,
) (map[uuid.UUID]*usecase.RoomDTO, error) {
	...
	result := lo.SliceToMap(out, func(item *usecase.RoomDTO) (uuid.UUID, *usecase.RoomDTO) {
		return item.Room.ID, item
	})
