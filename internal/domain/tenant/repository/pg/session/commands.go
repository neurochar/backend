...
func (r *Repository) Create(ctx context.Context, item *entity.Session) error {
	const op = "Create"
	...
	query, args, err := r.qb.Insert(pg.SessionTable).SetMap(dataMap).ToSql()
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, item *entity.Session) error {
	const op = "Update"
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		...
		updQuery, updArgs, err := r.qb.Update(pg.SessionTable).Where(
