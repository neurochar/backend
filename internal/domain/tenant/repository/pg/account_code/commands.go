...
func (r *Repository) Create(ctx context.Context, item *entity.AccountCode) error {
	const op = "Create"

	dataMap, err := dbhelper.DBModelToMap(pg.MapAccountCodeEntityToDBModel(item))
	if err != nil {
	...
	query, args, err := r.qb.Insert(pg.AccountCodeTable).SetMap(dataMap).ToSql()
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, item *entity.AccountCode) error {
	const op = "Update"
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		checkQuery, checkArgs, err := r.qb.Select("id").From(pg.AccountCodeTable).Where(squirrel.Eq{"id": item.ID}).ToSql()
		if err != nil {
		...
		updQuery, updArgs, err := r.qb.Update(pg.AccountCodeTable).Where(
