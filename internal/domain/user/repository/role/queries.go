...
func (r *Repository) buildWhereForList(listOptions *usecase.RoleListOptions, withDeleted bool) (where squirrel.And) {
	defer func() {
		if !withDeleted {
...
func (r *Repository) buildSortForList(_ *usecase.RoleListOptions) (sort []string) {
...
func (r *Repository) FindList(
	ctx context.Context,
	listOptions *usecase.RoleListOptions,
	queryParams *uctypes.QueryGetListParams,
) ([]*userEntity.Role, error) {
	const op = "FindList"
	...
	q := r.qb.Select(RoleTableFields...).From(RoleTable).Where(where).OrderBy(r.buildSortForList(listOptions)...)
	...
	result := make([]*userEntity.Role, 0, len(dbData))
...
func (r *Repository) FindOneByID(
	...
	q := r.qb.Select(RoleTableFields...).From(RoleTable).Where(where)
