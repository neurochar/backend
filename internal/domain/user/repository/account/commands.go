...
func (r *Repository) Create(ctx context.Context, item *userEntity.Account) error {
	const op = "Create"
...
func (r *Repository) Update(ctx context.Context, item *userEntity.Account) error {
	const op = "Update"
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		checkQuery, checkArgs, err := r.qb.Select("id").From(AccountTable).Where(squirrel.Eq{"id": item.ID}).ToSql()
		if err != nil {
		...
		updQuery, updArgs, err := r.qb.Update(AccountTable).Where(
