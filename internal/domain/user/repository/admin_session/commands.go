...
func (r *Repository) Create(ctx context.Context, item *userEntity.AdminSession) error {
	const op = "Create"
	...
	query, args, err := r.qb.Insert(SessionTable).SetMap(dataMap).ToSql()
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, item *userEntity.AdminSession) error {
	const op = "Update"
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		checkQuery, checkArgs, err := r.qb.Select("id").From(SessionTable).Where(squirrel.Eq{"id": item.ID}).ToSql()
		if err != nil {
		...
		updQuery, updArgs, err := r.qb.Update(SessionTable).Where(
