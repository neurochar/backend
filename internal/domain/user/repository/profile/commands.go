...
func (r *Repository) Create(ctx context.Context, item *userEntity.Profile) error {
	const op = "Create"
	...
	query, args, err := r.qb.Insert(ProfileTable).SetMap(dataMap).Suffix("RETURNING id").ToSql()
	if err != nil {
	...
	var id uint64
	err = r.pgClient.GetConn(ctx).QueryRow(ctx, query, args...).Scan(&id)
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, item *userEntity.Profile) error {
	const op = "Update"
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		checkQuery, checkArgs, err := r.qb.Select("id").From(ProfileTable).Where(squirrel.Eq{"id": item.ID}).ToSql()
		if err != nil {
		...
		updQuery, updArgs, err := r.qb.Update(ProfileTable).Where(
