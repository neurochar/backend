...
func (r *Repository) Create(ctx context.Context, item *entity.CandidateResume) error {
	const op = "Create"

	dataMap, err := dbhelper.DBModelToMap(pg.MapCandidateResumeEntityToDBModel(item))
	if err != nil {
	...
	query, args, err := r.qb.Insert(pg.CandidateResumeTable).SetMap(dataMap).ToSql()
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, item *entity.CandidateResume) error {
	const op = "Update"
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		checkQuery, checkArgs, err := r.qb.Select("id").From(pg.CandidateResumeTable).Where(squirrel.Eq{"id": item.ID}).ToSql()
		if err != nil {
		...
		updQuery, updArgs, err := r.qb.Update(pg.CandidateResumeTable).Where(
