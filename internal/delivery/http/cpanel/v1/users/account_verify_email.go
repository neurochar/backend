...
func (ctrl *Controller) AccountVerifyEmailHandler(c *fiber.Ctx) error {
	...
	err = ctrl.userFacade.Account.VerifyAccountEmailByCode(c.Context(), codeID, codeValue)
	if err != nil {
		return appErrors.Chainf(err, "%s.%s", ctrl.pkg, op)
	}

	return c.SendString(fmt.Sprintf(
		`E-mail подвержден. URL панели управления: %s`,
		ctrl.cfg.Global.CpanelFrontendUrl,
	))
}
